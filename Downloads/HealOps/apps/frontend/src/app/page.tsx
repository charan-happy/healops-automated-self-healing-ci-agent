          ...
          <div className="w-full rounded-2xl border border-white/[0.06] bg-gradient-to-b from-white/[0.03] to-transparent p-6 backdrop-blur-sm">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              {[
                ...
                const Tag = contributor.linkedin ? "a" : "span";
                const linkProps = contributor.linkedin ? { href: contributor.linkedin, target: "_blank", rel: "noopener noreferrer" } : {};
                return (
                  <Tag
                    key={contributor.name}
                    {...linkProps}
                    className={`inline-block rounded-full border border-white/[0.08] bg-white/[0.04] px-4 py-1.5 text-sm font-bold bg-gradient-to-r ${contributor.gradient} bg-clip-text text-transparent transition-all hover:scale-105 hover:border-white/[0.15] hover:bg-white/[0.08] hover:shadow-lg ${contributor.linkedin ? "cursor-pointer" : ""}`}
                  >
                    {contributor.name}
                  </Tag>
                );
