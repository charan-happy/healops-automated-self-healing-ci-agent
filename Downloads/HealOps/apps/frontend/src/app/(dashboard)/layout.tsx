            ...
            <div className="mx-auto max-w-5xl">
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 backdrop-blur-sm">
                ...
                <div className="flex flex-wrap items-center justify-center gap-2">
                  {contributors.map((c) => {
                    const Tag = c.linkedin ? "a" : "span";
                    const linkProps = c.linkedin ? { href: c.linkedin, target: "_blank", rel: "noopener noreferrer" } : {};
                    return (
                      <Tag
                        key={c.name}
                        {...linkProps}
                        className={`inline-block rounded-full border border-white/[0.08] bg-white/[0.04] px-3 py-1 text-xs font-bold bg-gradient-to-r ${c.gradient} bg-clip-text text-transparent transition-all hover:scale-105 hover:border-white/[0.15] hover:bg-white/[0.08] hover:shadow-lg ${c.linkedin ? "cursor-pointer" : ""}`}
                      >
                        {c.name}
                      </Tag>
                    );
