...
export async function fetchScmAvailableRepos(
  providerConfigId: string,
): Promise<{ provider: string; providerConfigId: string; repos: ScmAvailableRepo[]; error?: string } | null> {
  return fetchApi<{ provider: string; providerConfigId: string; repos: ScmAvailableRepo[]; error?: string }>(
